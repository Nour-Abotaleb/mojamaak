...
// Sample table data
const Finance = ref([
  { id: 1, apartments: 'A-10', name: "محمد العتيبي", all: '50,000 ريال', paid: '10,000', remaining: '40,000',  date: "2023-07-01", selected: false },
  { id: 2, apartments: 'A-10', name: "محمد العتيبي", all: '50,000 ريال', paid: '10,000', remaining: '40,000',  date: "2023-07-02", selected: false },
  { id: 3, apartments: 'A-10', name: "محمد العتيبي", all: '50,000 ريال', paid: '10,000', remaining: '40,000',  date: "2023-07-03", selected: false },
]);
...
const toggleAll = () => {
  Finance.value.forEach((b) => (b.selected = selectAll.value))
};

const toggleIndividual = () => {
  selectAll.value = Finance.value.every((b) => b.selected);
};
</script>
...
<template>
  <div class="bg-white dark:bg-[#1F2937] p-6 shadow-md">
    ...
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="text-xl font-bold text-gray-800 dark:text-white">إدارة الدفعات المالية</h2>
        <p class="text-gray-600 dark:text-gray-300">المدفوعات الشهرية للمجمع</p>
      </div>
      <button class="bg-[#38A6DE] text-white px-4 py-2 rounded hover:bg-[#2c8bbd]">إضافة دفعة مالية جديدة +</button>
    </div>
      ...
      <table class="min-w-full table-auto dark:border-gray-600">
        ...
        <tbody>
          ...
          >
            ...
            <td class="p-3 dark:text-gray-100">{{ b.all }}</td>
            <td class="p-3 dark:text-gray-100">{{ b.paid }}</td>
            <td class="p-3 dark:text-gray-100">{{ b.remaining }}</td>
