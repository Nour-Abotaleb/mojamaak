...
// Sample table data
const Messages = ref([
  { id: 1, apartments: 'A-10', name: "محمد العتيبي", content: 'استفسار عن الصيانة',  date: "2023-07-01", selected: false },
  { id: 2, apartments: 'A-10', name: "محمد العتيبي", content: 'استفسار عن الصيانة',  date: "2023-07-02", selected: false },
  { id: 3, apartments: 'A-10', name: "محمد العتيبي", content: 'استفسار عن الصيانة',  date: "2023-07-03", selected: false },
]);
...
const toggleAll = () => {
  Messages.value.forEach((b) => (b.selected = selectAll.value))
};

const toggleIndividual = () => {
  selectAll.value = Messages.value.every((b) => b.selected);
};
</script>
...
<template>
  <div class="bg-white dark:bg-[#1F2937] p-6 shadow-md">
    ...
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="text-xl font-bold text-gray-800 dark:text-white">إدارة الرسائل الواردة</h2>
        <p class="text-gray-600 dark:text-gray-300">جميع الرسائل التي أرسلهاالسكان إلي الإدارة</p>
      </div>
    </div>
      ...
      <table class="min-w-full table-auto dark:border-gray-600">
        ...
        <tbody>
          ...
          >
            ...
            <td class="p-3 dark:text-gray-100">{{ b.content }}</td>
            <td class="p-3 dark:text-gray-100">{{ b.date }}</td>
            <td class="p-3 flex gap-2">
              <button class="font-bold border rounded p-2 dark:text-gray-100 min-w-[70px]">قراءة</button>
